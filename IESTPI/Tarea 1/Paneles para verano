...
# Verano austral asociado a 2016: dic-2015, ene-2016, feb-2016
df_verano = df[
    (df["Fecha/Hora"] >= "2015-12-01 00:00:00") &
    (df["Fecha/Hora"] <  "2016-03-01 00:00:00")
].copy()

df_verano = df_verano.sort_values("Fecha/Hora").reset_index(drop=True)
...
eta = 0.5
h_convec = 5                  # W/m2K
A_e = (120*45 + 45*75*2 + 120*75) / 10**4   # m2


resultados = []

# SIMULACIÓN HORARIA DEL VERANO
for i in range(len(df_verano)):

    fecha_hora = df_verano.iloc[i]["Fecha/Hora"]
    rad = df_verano.iloc[i]["ghi"]                      # W/m2
    T_ambiente = df_verano.iloc[i]["temp"] + 273.15    # K
